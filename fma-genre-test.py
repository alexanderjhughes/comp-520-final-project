import os
os.environ["HF_AUDIO_DISABLE_TORCHCODEC"] = "1"
print("Torchcodec disabled:", os.environ.get("HF_AUDIO_DISABLE_TORCHCODEC"))
import ffmpeg
import torch
...
def ffmpeg_decode(audiobytes):
	#try except so that we don't try to decode corrupt audio files
	try:
		output, error = (
			ffmpeg.input("pipe:", format="mp3").output("pipe:", format="wav", acodec="pcm_f32le", ar = 16000, ac = 1).run(input=audiobytes, capture_stdout=True, capture_stderr=True, quiet = True)
		)
	except ffmpeg.Error as e:
		raise RuntimeError(f"FFmpeg decoding error: {e.stderr.decode()}")

	#transform into waveform
	waveform_encoding = np.frombuffer(output, np.float32)
    ...
    def __init__(self, data_dir):
        ...
        for data_item in train_data:
            ...
            try:
            	...
            	print("FAILED:", audio_item, "->", type(e), e)
